...
class ChatApplication:
    ...
    def __init__(self):
        self.window = Tk()
        self.window.iconbitmap(r'chatbot.ico')
    ...
    def _setup_main_window(self):
        self.window.title("Sophia")
        self.window.resizable(width=True, height=True)
        self.window.configure(width=470, height=550, bg=BG_COLOR)

        # head label
        head_label = Label(self.window, bg=BAR_COLOR, fg='red',
                           text="Hanoi University of Science and Technology",
                           font=FONT_BOLD, pady=10)
        head_label.place(relwidth=1)

        # tiny divider
        line = Label(self.window, width=450, bg=BAR_COLOR)
        ...
        # bottom label
        bottom_label = Label(self.window, bg=BAR_COLOR, height=80)
